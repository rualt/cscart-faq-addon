...
{if $view_mode != "button"}
    ...
    <div class="table-wrapper">
        ...
        {include file="addons/faq_page/pickers/faq_page/js.tpl" question_id="`$ldelim`question_id`$rdelim`" holder=$data_id input_name=$input_name clone=true hide_link=$hide_link hide_delete_button=$hide_delete_button position_field=$positions position="0"}
        {if $item_ids}
        {foreach name="items" from=$item_ids item="p_id"}
            {include file="addons/faq_page/pickers/faq_page/js.tpl" question_id=$p_id holder=$data_id input_name=$input_name hide_link=$hide_link hide_delete_button=$hide_delete_button first_item=$smarty.foreach.items.first position_field=$positions position=$smarty.foreach.items.iteration}
        {/foreach}
        {/if}
        </tbody>
